...
class DownloadsService {
  ...
  late final statusProvider =
      StreamProvider.family.autoDispose<DownloadItemStatus, (DownloadStub, int?)>((ref, record) {
  ...
  late final userDownloadedItemsProvider =
      StreamProvider.family.autoDispose<List<DownloadStub>, DownloadsScreenCategory>((ref, type) {
    return _isar.downloadItems
        .where()
  ...
  void syncItemDownloadSettings(DownloadItem item) {
    ...
    DownloadProfile bestProfile =
        transcodeProfiles.nonNulls.sorted((i, j) => ((j.quality - i.quality) * 1000).toInt()).first;
    if (!transcodeProfiles.nonNulls.contains(item.syncTranscodingProfile) ||
        (bestProfile.quality > (item.syncTranscodingProfile!.quality + 2000) && item.type != DownloadItemType.image)) {
  ...
  Future<List<BaseItemDto>> getCollectionTracks(BaseItemDto item, {bool playable = true}) async {
  ...
  Future<List<DownloadStub>> getAllCollections({
    ...
    return _isar.downloadItems
        ...
        .optional(fullyDownloaded, (q) => q.not().stateEqualTo(DownloadItemState.notDownloaded))
        .optional(onlyFavorites, (q) => q.anyOf(favoriteIds, (q, v) => q.isarIdEqualTo(v)))
        .optional(
            viewFilter != null,
            (q) => q.group((q) => q
