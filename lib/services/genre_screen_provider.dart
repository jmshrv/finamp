...
Future<
  ...
  CuratedItemSelectionType currentSelectionType = handleOfflineFallbackOption(
    ...
    currentFilter: (baseItemType == BaseItemDtoType.artist)
        ...
        : (baseItemType == BaseItemDtoType.album)
            ? ref.watch(
                finampSettingsProvider.genreCuratedItemSelectionTypeAlbums)
            : ref.watch(
                finampSettingsProvider.genreCuratedItemSelectionTypeTracks),
  ...
  Future<(List<BaseItemDto>, int)> fetchItems(
    ...
    if (isOffline) {
      final result = await getCuratedItemsOffline(
        ...
        library: library,
        artistInfoForType:
            (baseItemType == BaseItemDtoType.artist) ? artistInfoForType : null,
      );
      ...
      final result = await getCuratedItemsOnline(
        ...
        sortBySecondary: (baseItemType == BaseItemDtoType.album)
            ...
            : "SortName",
        artistType:
            (baseItemType == BaseItemDtoType.artist) ? defaultArtistType : null,
      );
      return (result.$1, result.$2);
    }
  }
...
Future<(List<BaseItemDto>, int)> getCuratedItemsOnline({
  ...
  final fetchedItems = await jellyfinApiHelper.getItemsWithTotalRecordCount(
    ...
    isFavorite:
        (genreCuratedItemSelectionType == CuratedItemSelectionType.favorites)
            ? true
            : null,
    limit: 5,
  ...
  if (genreCuratedItemSelectionType == CuratedItemSelectionType.favorites) {
    ...
    final fetchedItemCountWithoutFavorites =
        await jellyfinApiHelper.getItemsWithTotalRecordCount(
      parentItem: library,
      genreFilter: parent,
      limit: 1,
      includeItemTypes: baseItemType.idString,
...
Future<(List<BaseItemDto>, int)> getCuratedItemsOffline({
  ...
  final List<DownloadStub> fetchedItems = (baseItemType == BaseItemDtoType.track)
      ? await downloadsService.getAllTracks(
          ...
          onlyFavorites: (genreCuratedItemSelectionType ==
      ...
      : await downloadsService.getAllCollections(
          ...
          viewFilter:
              (baseItemType == BaseItemDtoType.album) ? library?.id : null,
  ...
  if (genreCuratedItemSelectionType == CuratedItemSelectionType.favorites) {
    final List<DownloadStub> allFetchedItems = (baseItemType ==
            BaseItemDtoType.track)
        ? await downloadsService.getAllTracks(
            nullableViewFilters: ref
                .read(finampSettingsProvider.showDownloadsWithUnknownLibrary),
            genreFilter: parent)
        : await downloadsService.getAllCollections(
            baseTypeFilter: baseItemType,
            fullyDownloaded:
