...
class QueueService {
      ...
      if ((_saveUpdateCycleCount >= 5 && !_audioHandler.paused) || _saveUpdateImmediate) {
        ...
        if (_savedQueueState == SavedQueueState.saving) {
          ...
          FinampStorableQueueInfo info =
              FinampStorableQueueInfo.fromQueueInfo(getQueue(), _audioHandler.playbackPosition.inMilliseconds);
  ...
  Future<void> loadSavedQueue(FinampStorableQueueInfo info) async {
    ...
    try {
      ...
      List<jellyfin_models.BaseItemId> missingIds = allIds.toSet().difference(playlistIds).toList();

      if (FinampSettingsHelper.finampSettings.isOffline) {
        for (var id in missingIds) {
          jellyfin_models.BaseItemDto? item = downloadsService.getTrackDownload(id: id)?.baseItem;
          if (item != null) {
            idMap[id] = item;
          }
        }
  ...
  Future<void> addNext({
    ...
    try {
      ...
      for (final queueItem in queueItems.reversed) {
        ...
        _queueServiceLogger
            .fine("Appended '${queueItem.item.title}' to Next Up (index ${adjustedQueueIndex + offset})");
      }
  ...
  Future<Uri> _trackUri(MediaItem mediaItem) async {
    ...
    if (mediaItem.extras!["shouldTranscode"] as bool) {
      ...
      queryParameters.addAll({
        ...
        "audioSampleRate":
            FinampSettingsHelper.finampSettings.transcodingStreamingFormat.codec == 'opus' ? '48000' : '44100',
        "maxAudioBitDepth": "16",
...
class NextUpShuffleOrder extends ShuffleOrder {
  ...
  void shuffle({int? initialIndex}) {
    ...
    indices =
        List.generate(queueInfo.previousTracks.length + 1 + queueInfo.nextUp.length + queueInfo.queue.length, (i) => i);
