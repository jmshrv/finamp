  ...
  Widget build(BuildContext context) {
    ...
    void playAllFromArtist(List<BaseItemDto> items) {
      queueService.startPlayback(
        ...
        source: QueueItemSource(
          type: isGenre ? QueueItemSourceType.genre : QueueItemSourceType.artist,
    ...
    void shuffleAllFromArtistNext(List<BaseItemDto> items) {
      ...
      queueService.addNext(
        items: shuffled,
        source: QueueItemSource(
          type: isGenre ? QueueItemSourceType.genre : QueueItemSourceType.nextUpArtist,
          name: QueueItemSourceName(
              type: QueueItemSourceNameType.preTranslated,
              pretranslatedName: parentItem.name ?? AppLocalizations.of(context)!.placeholderSource),
          id: parentItem.id,
    ...
    void addArtistNext(List<BaseItemDto> items) {
      queueService.addNext(
          items: items,
          source: QueueItemSource(
            type: isGenre ? QueueItemSourceType.nextUpPlaylist : QueueItemSourceType.nextUpArtist,
            name: QueueItemSourceName(
                type: QueueItemSourceNameType.preTranslated,
                pretranslatedName: parentItem.name ?? AppLocalizations.of(context)!.placeholderSource),
            id: parentItem.id,
      ...
      GlobalSnackbar.message((scaffold) => AppLocalizations.of(scaffold)!.confirmPlayNext(isGenre ? "genre" : "artist"),
          isConfirmation: true);
    }
    ...
    void addArtistToNextUp(List<BaseItemDto> items) {
      ...
      GlobalSnackbar.message(
          (scaffold) => AppLocalizations.of(scaffold)!.confirmAddToNextUp(isGenre ? "genre" : "artist"),
          isConfirmation: true);
    }
    ...
    void addArtistToQueue(List<BaseItemDto> items) {
      queueService.addToQueue(
          items: items,
          source: QueueItemSource(
            type: isGenre ? QueueItemSourceType.nextUpPlaylist : QueueItemSourceType.artist,
            name: QueueItemSourceName(
                type: QueueItemSourceNameType.preTranslated,
                pretranslatedName: parentItem.name ?? AppLocalizations.of(context)!.placeholderSource),
            id: parentItem.id,
      ...
      GlobalSnackbar.message(
          (scaffold) => AppLocalizations.of(scaffold)!.confirmAddToQueue(isGenre ? "genre" : "artist"),
          isConfirmation: true);
    }
                              ...
                              onSelected: (ArtistMenuItems selection) async {
                                switch (selection) {
                                  case ArtistMenuItems.playNext:
                                    allTracks.then((items) => addArtistNext(items ?? []));
                                    break;
                                  case ArtistMenuItems.addToNextUp:
                                    allTracks.then((items) => addArtistToNextUp(items ?? []));
                                    break;
                                  case ArtistMenuItems.shuffleNext:
                                    allTracks.then((items) => shuffleAllFromArtistNext(items ?? []));
                                    break;
                                  case ArtistMenuItems.shuffleToNextUp:
                                    allTracks.then((items) => shuffleAllFromArtistToNextUp(items ?? []));
                                    break;
                                  case ArtistMenuItems.addToQueue:
                                    allTracks.then((items) => addArtistToQueue(items ?? []));
                                    break;
                                  case ArtistMenuItems.shuffleToQueue:
                                    allTracks.then((items) => shuffleAllFromArtistToQueue(items ?? []));
                                    break;
                                  case ArtistMenuItems.shuffleAlbums:
                                    allTracks.then((items) => shuffleAlbumsFromArtist(items ?? []));
                                    break;
                                  case ArtistMenuItems.shuffleAlbumsNext:
                                    allTracks.then((items) => shuffleAlbumsFromArtistNext(items ?? []));
                                    break;
                                  case ArtistMenuItems.shuffleAlbumsToNextUp:
                                    allTracks.then((items) => shuffleAlbumsFromArtistToNextUp(items ?? []));
                                    break;
                                  case ArtistMenuItems.shuffleAlbumsToQueue:
                                    allTracks.then((items) => shuffleAlbumsFromArtistToQueue(items ?? []));
                                    break;
                                  default:
                                    break;
                                }
                              ...
                              itemBuilder: (context) {
                                ...
                                return <PopupMenuEntry<ArtistMenuItems>>[
                                  ...
                                  PopupMenuItem<ArtistMenuItems>(
                                    ...
                                    child: ListTile(
                                      leading: const Icon(TablerIcons.playlist),
                                      title: Text(AppLocalizations.of(context)!.shuffleAlbums),
                                    ),
                                  ),
                                  if (queueService.getQueue().nextUp.isNotEmpty)
                                    PopupMenuItem<ArtistMenuItems>(
                                      value: ArtistMenuItems.shuffleAlbumsNext,
                                      child: ListTile(
                                        leading: const Icon(TablerIcons.corner_right_down),
                                        title: Text(AppLocalizations.of(context)!.shuffleAlbumsNext),
                                      ),
                                    ),
                                  PopupMenuItem<ArtistMenuItems>(
                                    value: ArtistMenuItems.shuffleAlbumsToNextUp,
                                    child: ListTile(
                                      leading: const Icon(TablerIcons.corner_right_down_double),
                                      title: Text(AppLocalizations.of(context)!.shuffleAlbumsToNextUp),
                                    ),
                                  ),
                                  PopupMenuItem<ArtistMenuItems>(
                                    value: ArtistMenuItems.shuffleAlbumsToQueue,
                                    child: ListTile(
                                      leading: const Icon(TablerIcons.playlist),
                                      title: Text(AppLocalizations.of(context)!.shuffleAlbumsToQueue),
                                    ),
                                  ),
                                ];
                              },
