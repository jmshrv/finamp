  ...
  Widget build(BuildContext context) {
    ...
    if (isOffline) {
      ...
      futures = Future.wait([
        ...
        Future.sync(() async {
          final List<DownloadStub> artistAlbums = await _downloadsService.getAllCollections(
      ...
      allTracks = Future.sync(() async {
        ...
        final List<DownloadStub> albumArtistAlbums = await _downloadsService.getAllCollections(
            baseTypeFilter: BaseItemDtoType.album,
            relatedTo: widget.parent,
            artistType: (widget.parent.type == "MusicGenre") ? null : ArtistType.albumartist);
        ...
        for (var album in albumArtistAlbums) {
          sortedTracks.addAll(await _downloadsService.getCollectionTracks(album.baseItem!, playable: true));
        }
        ...
        final List<DownloadStub> performingArtistAlbums = await _downloadsService.getAllCollections(
            baseTypeFilter: BaseItemDtoType.album,
            relatedTo: widget.parent,
            artistType: (widget.parent.type == "MusicGenre") ? null : ArtistType.artist);
    ...
    return FutureBuilder(
        ...
        builder: (context, snapshot) {
          ...
          var appearsOnAlbums = albumsAsPerformingArtist.where((a) => !albums.any((b) => b.id == a.id)).toList();
          var topTracks = tracks.takeWhile((s) => (s.userData?.playCount ?? 0) > 0).take(5).toList();
          ...
          return PaddedCustomScrollview(slivers: <Widget>[
            SliverAppBar(
              ...
              expandedHeight:
                  widget.parent.type != "MusicGenre" ? kToolbarHeight + 125 + 96 : kToolbarHeight + 125 + 16,
              pinned: true,
              ...
              actions: [
                ...
                if (widget.parent.type != "MusicGenre") FavoriteButton(item: widget.parent),
                if (!_isLoading)
                  DownloadButton(
                      item: DownloadStub.fromItem(item: widget.parent, type: DownloadItemType.collection),
                      children: (widget.parent.type == "MusicGenre") ? albums : allChildren)
              ],
            ),
            if (!_isLoading && !isOffline && ref.watch(finampSettingsProvider.showArtistsTopTracks))
              TopTracksSection(parent: widget.parent, topTracks: topTracks, childrenForQueue: Future.value(tracks)),
            if (albums.isNotEmpty)
              AlbumSection(parent: widget.parent, albumsText: AppLocalizations.of(context)!.albums, albums: albums),
