...
Since the app is not available via the Microsoft Store yet, you'll have to install a self-signed certificate before you can install the `.msix` generated by the CI pipeline.  
To do that, [download this certificate](https://github.com/jmshrv/finamp/raw/refs/heads/redesign/windows/runner/resources/MSIX_LOCAL_CERTIFICATE.pfx), and install it for `Local Machine`, using the password `finamp`, into the `Trusted Root Certification Authorities` store.  
Then you can download the latest `.msix` file from the repo and install it.
